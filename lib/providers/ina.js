...
var getUrl = function getUrl(videoId) {
  return "//player.ina.fr/player/embed/" + videoId + "/1/1b0bd203fbcd702f9bc9b10ac3d0fc21/wide/0";
};
...
var walker = function walker(xmlObj, path) {
  ...
  return walker(_.find(xmlObj.childs, {
    name: path.shift()
  }), path);
};
...
var provider = {
  ...
  label: 'INA',
  headers: {},
  videoIdExtractRegExps: [// ex. http://www.ina.fr/video/NA00001285844/monopoly-nantais-video.html
  ...
  getPublishedDate: function getPublishedDate(videoId) {
    return fetchVideo("//player.ina.fr/notices/" + videoId + ".mrss").then(function (res) {
      return find(xml(res), 'channel.pubDate').childs[0];
    });
  },
