...
var provider = {
  ...
  mdtk: null,
  mainCatalog: null,
  videoIdExtractRegExps: [// ex. http://www.ultimedia.com/default/index/videogeneric/id/5ss8kq
  /^(?:https?:)?\/\/(?:www\.)?ultimedia\.com\/default\/index\/videogeneric\/id\/([^?&#/]+)/i, // ex. http://www.digiteka.net/default/index/videogeneric/id/550sru
  /^(?:https?:)?\/\/(?:www\.)?digiteka\.net\/default\/index\/videogeneric\/id\/([^?&#/]+)/i, // ex. http//www.ultimedia.com/deliver/generic/iframe/mdtk/01637594/zone/34/src/5lfqkv
  ...
  getPublishedDate: function getPublishedDate(videoId) {
    return fetchVideo(videoId).then(function (res) {
      return _.get(res, 'results.creation_date');
    });
  },
  ...
  searchUrl: searchUrl,
  search: function search(query, offset) {
    if (offset === void 0) {
      offset = 0;
    }

    return fetch(searchUrl(query, provider.itemsPerPage, 1 + Math.ceil(offset / provider.itemsPerPage)), {
      timeout: 10000,
      headers: provider.headers
    }).then(function (res) {
      return res.json();
    }).then(function (res) {
      return _.get(res, 'results', []).map( // eslint-disable-next-line
      function (_ref) {
