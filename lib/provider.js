...
var providers = [dailymotion, ina, youtube, digiteka];
...
function getVideoFromId(provider, videoId) {
  return Promise.all([provider.getTitle(videoId), provider.getDescription(videoId), provider.getDuration(videoId), provider.getThumbnailUrl(videoId), provider.getPlayerUrl(videoId)]).then(function (_ref) {
    ...
    return formatter(provider.name, videoId, { title: title, description: description, duration: duration, thumbnailUrl: thumbnailUrl, playerUrl: playerUrl });
  });
}
