...
const fetchVideo = _.memoize(videoId =>
  fetch(fetchUrl(videoId), { headers: provider.headers }).then(res =>
    res.json()
  )
);
...
const extractThumbnailUrl = imageUrl =>
  _.nth(/(?:https?:)(.*)/.exec(imageUrl), 1);

const extractPlayerUrl = iframeTag =>
  _.nth(/src="(?:https?:)([^"]*)/.exec(iframeTag), 1);
...
const provider = {
  ...
  getThumbnailUrl: videoId =>
    fetchVideo(videoId).then(res =>
      extractThumbnailUrl(_.get(res, "results.image_high"))
    ),

  getTitle: videoId =>
    fetchVideo(videoId).then(res => _.get(res, "results.title")),

  getDescription: videoId =>
    fetchVideo(videoId).then(res => _.get(res, "results.description")),
  ...
  search: (query, offset = 0) =>
    fetch(
      searchUrl(
        ...
        1 + Math.ceil(offset / provider.itemsPerPage)
      ),
